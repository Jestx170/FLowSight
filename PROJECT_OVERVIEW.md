...
│  ├─ templates/index.html   ← React build — **เสิร์ฟที่ `/` แล้ว** (legacy index.html ถูกลบ)
...
> dev ยังใช้ :8080 (HMR) เหมือนเดิม; prod ใช้ build ที่ฝังใน image (`templates/index.html` + `static/assets/`)
...
| `src/styles.css` | theme tokens (Odoo: `--primary #714B67`) |
| `backend/templates/index.html` + `backend/static/assets/` | React build ที่ถูก deploy — เสิร์ฟที่ `/` |
